...
float calcDirShadow(vec4 fragPosLightSpace, vec3 normal, vec3 lightDir, float dirLightIndex)
{
	...
	for (int x = -1; x <= 1; ++x)
	{
		for (int y = -1; y <= 1; ++y)
		{
			float pcfDepth = texture(u_dirShadowMaps, vec3(clipSpaceCoords.xy + vec2(x, y) * texelSize, dirLightIndex)).r;
...
float calcPointShadow(vec3 fragPos, vec3 lightPos, vec3 normal, float viewDistance, float pointLightIndex)
{
	...
	{
		float closestDepth = texture(u_pointShadowMaps, vec4(dir + sampleOffsetDirs[i] * diskRad, pointLightIndex)).r * zFar;
...
void main()
{
	...
	if (normal.xyz != vec3(0.0f, 0.0f, 0.0f))
	{
		...
		{
			float shadow = 1.0f - calcDirShadow(dirLights[i].toLightSpace * fragPos, normal.xyz, -dirLights[i].direction, float(i));
