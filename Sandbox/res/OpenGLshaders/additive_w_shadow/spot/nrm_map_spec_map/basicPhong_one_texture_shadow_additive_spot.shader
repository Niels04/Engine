...
vec3 calcSpotLight(spotLight light, vec3 normal, vec3 fragPos, vec3 viewDir, vec3 texColor, float shadow, vec3 specIntensity)//maybe also add attenuation calculation to spotlights in order to have flashLights that fade over distance
{
...
void main()
{
	...
	colOutput = calcSpotLight(sptLights[u_lightIndex], normal, v_fragPos, viewDir, col_interpolate, shadow, specIntensity);
