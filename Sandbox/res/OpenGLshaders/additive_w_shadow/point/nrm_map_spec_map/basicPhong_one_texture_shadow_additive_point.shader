...
vec3 calcPointLight(pointLight light, vec3 normal, vec3 fragPos, vec3 viewDir, vec3 texColor, float shadow, vec3 specIntensity)
{
	...
	//specular
	vec3 specular = light.specular.xyz * specIntensity * pow(max(0.0f, dot(reflect(-lightDir, normal), viewDir)), shininess) * attenuation * shadow;
...
void main()
{
	...
	//shadow = 1.0f;
	colOutput = calcPointLight(pLights[u_lightIndex], normal, v_fragPos, viewDir, col_interpolate, shadow, specIntensity);
