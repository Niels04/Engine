...
vec3 calcDirLight(directionalLight light, vec3 normal, vec3 viewDir, vec3 texColor, float shadow, vec3 specIntensity)
{
	...
	//specular
	vec3 specular = light.specular.xyz * specIntensity * pow(max(0.0f, dot(reflect(light.direction, normal), viewDir)), shininess) * shadow;//when using"reflect()" put in the vector from the lightSource towards the fragment
