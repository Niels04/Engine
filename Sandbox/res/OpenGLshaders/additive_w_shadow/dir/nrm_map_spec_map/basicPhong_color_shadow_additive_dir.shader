...
vec3 calcDirLight(directionalLight light, vec3 normal, vec3 viewDir, float shadow, vec3 specIntensity)
{
	...
	//specular
	vec3 specular = light.specular.xyz * specIntensity * pow(max(0.0f, dot(reflect(light.direction, normal), viewDir)), shininess) * shadow;//when using"reflect()" put in the vector form the lightSource towards the fragment
...
void main()
{
	...
	colOutput = calcDirLight(dirLights[u_lightIndex], normal, viewDir, shadow, specIntensity);
